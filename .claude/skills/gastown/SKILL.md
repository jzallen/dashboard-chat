...
| `references/headless-merge-queue.md` | Operational recipe for running ONLY Dolt + Refinery (skip Mayor / Witness / Deacon / Daemon / polecats / convoys) | When the user wants the merge queue mechanic without the rest of Gas Town — typical phrasing: "just the refinery", "headless merge queue", "minimal merge queue", or complaints about token overhead from the orchestration layer |
