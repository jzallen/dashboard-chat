"""add assistant_audit_entries spine + reversed FK on transforms

Revision ID: d7e8f9a0b1c2
Revises: c6d7e8f9a0b1
Create Date: 2026-06-06 00:00:00.000000

AssistantAuditEntry (rich-catalog-backend-models §2.3 / §2.10): a GENERIC
assistant-audit spine — scoping columns (``org_id``/``project_id``/``node_id``/
``node_kind``) plus a single JSON ``payload`` carrying the variable content
(``{tool, say, tag, args?}``). NO per-subtype columns. The FK is REVERSED: the
detail (``Transform``) points UP at the spine via
``transforms.assistant_audit_entry_id``, not the other way round.
...
than deleting it when its audit entry is removed.
"""
...
def upgrade() -> None:
    ...
    op.create_index("ix_assistant_audit_entries_org_id", "assistant_audit_entries", ["org_id"])
    ...
    with op.batch_alter_table("transforms", schema=None) as batch_op:
        batch_op.add_column(sa.Column("assistant_audit_entry_id", sa.Text(), nullable=True))
        batch_op.create_foreign_key(
            "fk_transforms_assistant_audit_entry_id",
            "assistant_audit_entries",
            ["assistant_audit_entry_id"],
            ["id"],
            ondelete="SET NULL",
        )
        batch_op.create_index("ix_transforms_assistant_audit_entry_id", ["assistant_audit_entry_id"])


def downgrade() -> None:
    with op.batch_alter_table("transforms", schema=None) as batch_op:
        batch_op.drop_index("ix_transforms_assistant_audit_entry_id")
        batch_op.drop_constraint("fk_transforms_assistant_audit_entry_id", type_="foreignkey")
        batch_op.drop_column("assistant_audit_entry_id")
