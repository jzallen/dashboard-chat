...
class TestMacrosSql:
    def test_generate_macros_when_called_contains_jinja_macro_block(self):
        output = generate_macros_sql()
        assert "{% macro register_custom_functions() %}" in output
        assert "{% endmacro %}" in output

    def test_generate_macros_when_called_contains_title_case_macro(self):
        output = generate_macros_sql()
        assert "CREATE OR REPLACE MACRO title_case(s)" in output

    def test_generate_macros_when_called_contains_snake_case_macro(self):
        output = generate_macros_sql()
        assert "CREATE OR REPLACE MACRO snake_case(s)" in output

    def test_generate_macros_when_called_contains_kebab_case_macro(self):
        output = generate_macros_sql()
        assert "CREATE OR REPLACE MACRO kebab_case(s)" in output

    def test_generate_macros_when_called_registers_via_run_query(self):
        output = generate_macros_sql()
