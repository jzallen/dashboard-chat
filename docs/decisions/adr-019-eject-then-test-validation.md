...
> **Status update (2026-05-11) — partially superseded by [ADR-024](./adr-024-rebalance-dbt-test-validation.md).**
> The per-flow eject mechanism (this ADR's binding point β) is preserved in concept — the customer's exported dbt project still IS the validation surface — but its harness-facade-driven realization (the `EjectAndTestOrchestrator` family, 5 Earned-Trust probes, and `harness.eject_and_test`) was rebalanced toward a procedure-shaped dbt-test driver after the maintenance-load profile of the original design exceeded its scenario-count value. The Earned-Trust contract (D2 — substrate lies must surface loudly, not silently) is satisfied by inline failure modes at the point of use in the v2 driver rather than session-scoped probes. The customer-fidelity invariant (O4 — validation logic durable across the in-app ↔ ejected-dbt boundary) survives via the v2 driver's exported `seeded_profile_bucket` / `seeded_profile_endpoint` fields. The original Status/Date/Ratified lines below stand as the historical record. See ADR-024 and [migration-roadmap.md](../feature/rebalance-dbt-test-validation/design/migration-roadmap.md) for the rebalance rationale, phase plan, and outcomes.
