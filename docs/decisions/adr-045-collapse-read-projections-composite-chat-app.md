...
**Status:** **Superseded by [ADR-046](adr-046-stateproxy-actor-surface.md) (2026-05-29).** The proxy model (`StateProxy` client + a single server-resident-actor `/state` surface) plus the FE-rewrite mandate removed the byte-frozen `FlowProjection` contract this ADR's composite-of-three-envelopes design — and its ADR-040-style additive-alias migration — were built around. The three per-machine projections become client-side `useSelector` selectors over ONE state document; see ADR-046 §"Why this supersedes ADR-045". The analysis below (the A1/A2/A3 read-shape trade-off, the SSE-consolidation evidence) stays on record as history and is reused by ADR-046.

_Originally:_ Proposed (DESIGN, propose-mode — options presented with a recommendation; awaiting user ratification before DELIVER). Passed the nw-design gate review (nw-solution-architect-reviewer, 2026-05-29): all three blocking design items — composite mount mechanism (CRITICAL-1), composite mapper/SSE seam + `sequence_id` semantics (CRITICAL-2/HIGH-2), and auth-proxy sniffer shape-detection (HIGH-3) — confirmed RESOLVED in specification, no new issues. The reviewer's residual code-correspondence checks (sniffer ternary in `auth-proxy/app.ts`, the new mapper/type/router) are **DELIVER-wave work**, itemized in the phased plan (MR-A…MR-G); this is a DESIGN-only artifact and intentionally lands no code.
**Date:** 2026-05-29
