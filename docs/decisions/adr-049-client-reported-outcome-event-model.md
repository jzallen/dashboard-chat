...
**Status:** Accepted (user-ratified 2026-06-11; DR-2 overridden at ratification — outcome events use plain past tense, not the `*_reported` suffix this pass recommended; all other DR-1–DR-8 recommendations ratified as proposed)
**Date:** 2026-06-10
...
- **Onboarding:** `session_begin` (kept) · `org_found` · `org_not_found` · `org_created` · `org_create_failed{cause}` · `__force_failure__` (kept, gated).
- **Project-context:** `scope_resolved` · `no_projects_found` · `scope_mismatch{cause}` · `project_created` · `project_create_failed{cause}` · `project_switched` · `open_deep_link` (kept, wish-capture) · `back_to_projects_clicked` (kept).

**Naming (user-ratified, overriding this pass's `*_reported`-suffix recommendation): plain past tense.** ui-state is an **extension of the frontend** — the same trust domain, existing to persist presentation state across re-entry (tab-crash recovery is deferred; every re-entry currently starts at login) — so the event language reads as **the flow the user is following** through the app, and the event stream doubles as an **audit trail in the client console logs** (structured `createLogger`: each posted event and resulting region state is logged). The "client observation admitted under the PCO invariant" posture lives in this ADR + INV-PCO (§Decision 2), not in a per-event suffix. The homonym with the retired server-verified FlowEvent vocabulary is temporal only — that vocabulary retires in this same feature, so the two trust regimes never coexist on the wire.
...
| D5 (state set) | **Amended:** `verifying` → `awaiting_org_report`; `creating_org` retired; `error_recoverable` kept by name and made genuinely recoverable; `session_rejected` retired (producer gone); `[hasOrg]` survives as the `org_found` fast path. |
...
- **Onboarding:** `awaiting_org_report → ready | needs_org` on probe reports; `needs_org → ready` on `org_created`; `org_create_failed` splits **re-edit vs. retry** — `org_name_taken` stays in `needs_org` with the inline validation error (today's 409 arm preserved), every other cause lands in `error_recoverable`, which **accepts all outcome reports** (Spec 5: a compensated or even orphaned ADR-048 failure is retried by re-submission — no dead ends, the `partial-setup` terminal dies). Report acceptance is wide across pre-`ready` states so multi-tab/crash flows **converge**; `ready` ignores late reports and answers with the current document.
- **Project-context:** `awaiting_scope_report → project_selected | no_projects | scope_mismatch_terminal` on reports; Phase D (the automatic default project) is `project_created` accepted from `awaiting_scope_report` or `no_projects`; failures land in a report-accepting `error_recoverable`; switching is report-only (`project_switched` / `scope_mismatch`). The last-used-resolution and deep-link-discrimination policies move to the client with the retired resolvers.
...
- One write philosophy across the product (client-driven optimistic write-through); the event names read as the user's flow narration, and the event stream doubles as the console-log audit trail (the trust posture is carried by INV-PCO + this ADR, per the ratified naming decision).
