...
**Status:** Accepted (user-ratified 2026-06-11, with two amendments: outcome events use plain past tense per the ADR-049 DR-2 override, and cause tags are machine-readable only — the UI never renders a raw cause string, see (c))
**Date:** 2026-06-10
...
### (b) Org-id carry: the identity header `X-Org-Id`, strip-then-inject

The interception branch, after a successful WorkOS org-create, **overrides** `X-Org-Id: <workos_org_id>` on the forwarded `POST /api/orgs` (the caller's `X-Org-Id` is empty — a first-org creator has no org). `X-Org-Id` is already an `IDENTITY_HEADER` (`auth-proxy/lib/auth.ts`), stripped-then-injected on every route, so a client-supplied value is unforgeable by the same construction that protects `X-User-Id`. The backend resolves it into `user.org_id` (`get_current_user`) and the create use case passes it to the repository as the row `id=`, preserving the rule that **the WorkOS org id IS the local org id**. Claim absent/None (dev mode, and all non-intercepted traffic) → backend-generated id.

**No separate `X-Provisioned-Org-Id`, and no "user already has an org" guard.** An earlier revision carried the new id in a dedicated `X-Provisioned-Org-Id` header alongside a `_ensure_user_has_no_org` guard that inspected the caller's *current* `X-Org-Id`. That distinction was unnecessary: both are auth-proxy-injected, client-stripped headers (identical trust), and on the create route the caller by definition has no prior org — the guard only ever misfired (an empty `X-Org-Id` read as "already has an org"). Collapsed to a single `X-Org-Id` carry; uniqueness is enforced by the global org-name index (`OrganizationNameTakenError` → 409) and the org-id primary key.

Considered and rejected: proxy body rewrite (breaks the streamed forward, and conflates client-authored with proxy-authored body fields); backend accepting `id` in the request body (client-forgeable org-id minting — violates the headers-not-bodies trust posture of ADR-016).
...
Cause enums (closing ADR-049 §3.2's delegation): `OrgCreateFailureCause = "org_name_taken" | "org_name_invalid" | "org_create_failed"`; `ProjectCreateFailureCause = "project_create_failed"`; `ScopeMismatchCause` unchanged. Client mapping: `201 → org_created`; `409 → org_name_taken`; `400/422 → org_name_invalid`; anything else (incl. network/timeout) → `org_create_failed`; `401` is never an outcome report (auth gate). The two-way domain split holds: the first two re-edit in `needs_org` (inline `org_validation_error`), the third lands in the report-accepting `error_recoverable`. (Naming note: after the DR-2 override the generic cause tag and the failure *event* share the spelling `org_create_failed`; they live in different fields — `type` vs. `payload.cause` — and never collide structurally.)

**Display rule (user-ratified amendment):** cause tags are **machine-readable wire values only — never rendered to the user**. The UI maps them to copy it owns: re-edit causes (`org_name_taken`, `org_name_invalid`) become user-friendly inline helper text on the org form; everything else (`org_create_failed`, `project_create_failed`) triggers a distinct "something went wrong on our end" presentation with the retry affordance. Raw tags may appear in the console-log audit trail (ADR-049), never in rendered copy — the shipped `Cause: partial-setup` rendering (`onboarding.tsx`) is the anti-pattern this kills.

Retry policy: **manual only** for org create (non-idempotent end-to-end — ADR-048 R5); **probe-first convergence** for the default project (re-probe `GET /api/projects` before re-POSTing — a lost 201 becomes `scope_resolved`, not a duplicate). **Probe transport failures are not reportable**: only definitive SSOT answers (`200`/`404`) produce `org_found`/`org_not_found` — the earned-trust rule applied client-side.
...
- **`ChatAppWireEvent` becomes a closed union** (full TS sketch in the deliverable §e.1): the ADR-049 outcome members with the (c) cause enums and `{id, name}` display snapshots; kept members `session_begin`, `open_deep_link`, `back_to_projects_clicked`, `__force_failure__`; the session-chat vocabulary (executing DR-8): outcome members `session_list_loaded`, `session_resumed`, `session_created`, `dataset_context_switched` (+ `*_failed` partners) and the surviving UI intents (`session_clicked`, `new_session_clicked`, `first_message_sent`, `refresh_session_list`, `dataset_resolved_by_agent`, `dataset_picked_directly`, suggestion chips). Retired: `org_form_submitted`, `create_project_submitted` (UI-1 dies), `create_project_clicked`, `switching_project_intent`, `retry_clicked`, and the `{type: string}` catch-all (`wire-event.ts:52`).
