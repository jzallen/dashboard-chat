...
**Negative / debt**
- **This is the 2nd instance of the Cold-Storage pattern** (dataset was 1st). `RETENTION_WINDOW = timedelta(days=90)` and the archive/restore/filter logic are now duplicated across the dataset and source paths. This ADR **does not** extract shared abstraction yet — and deliberately **waits for the rule of three** (a 3rd instance) before doing so; duplicating twice is cheaper than the wrong abstraction. When a 3rd resource needs Cold Storage, prefer a **`Protocol`** (structural, consumer-defined — the code that needs the interface declares it) over an inheritance mixin/base class or `functools.singledispatch`: it is the simplest, keeps the domain models free of shared inheritance, and lets each aggregate satisfy the contract by shape. A shared `RETENTION_WINDOW` constant can be lifted independently and sooner if desired. A future `/nw-refactor` owns this.
