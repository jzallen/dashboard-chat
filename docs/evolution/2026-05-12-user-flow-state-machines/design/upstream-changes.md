...
**DESIGN state**: ADR-027 ratifies this verbatim; the agent is unmodified. The flow-state tier is a NEW Hono service in compose, peer to the agent.
...
**DESIGN state**: **No new ADR for persistence.** ADR-027 §3 explicitly inherits ADR-018's policy verbatim: `REDIS_URL` set → Redis tier; unset → noop. New key prefix (`flow:{flow_id}:events`); same compose Redis container; same `selectThreadPersister`-shaped dispatch helper.
...
- Server-resolved at the flow-state tier's ScopeResolver.
...
- The DEVOPS instrumentation list from DISCUSS `outcome-kpis.md` is unchanged; this feature ADDS flow-state-tier-specific events (see `handoff-design-to-distill.md` §"Instrumentation list").
...
**Morgan's state**: `application-architecture.md` §"Key callouts" claims "Auth-proxy is the only ingress for the FE, the harness, the flow-state tier, the agent, and the backend. ADR-016 is honored."
...
**Resolution**: ADR-030 ratifies routing the **flow-state tier** behind auth-proxy from day 1 (honoring ADR-016 for the new tier). The agent's bypass is documented as a pre-existing inconsistency; ADR-030 does not perpetuate it but also does not fix it (out of scope). The flow-state tier sits behind auth-proxy correctly from PR-0.
...
**Morgan's state**: ADR-027 §3 specifies the Redis key prefix as `flow:{flow_id}:events` but leaves `flow_id` schema open. The implication in the seed example (`flow_id = "login-and-org-setup"`) suggests machine-name-only is acceptable.

**Titan's state**: this is **multi-tenant-unsafe**. A `freeze` event broadcast to actors under `flow:loginAndOrgSetup:events` would freeze ALL users' login flows. ADR-030 amends:
...
**Morgan's state**: `application-architecture.md` §11 claims "(was 5; +1 for flow-state)" → 6 services.
