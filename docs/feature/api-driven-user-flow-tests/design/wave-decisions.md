...
- **[D10] Tool-dispatcher mimics the frontend's responsibility.** A Python `ToolCallDispatcher` interprets `applyCleaningTransform` tool-calls from the SSE stream and POSTs to `/api/datasets/{id}/transforms`. This is a code-level discovery DISCUSS did not surface. Recorded as `upstream-changes.md` to ensure the AC's "10 cleanup ops" expectation is grounded in two-step LLM behavior (preview tool + applyCleaningTransform).
...
- **Key new components**:
  - `backend/tests/integration/dataset_layer/harness.py` (NEW) — `DatasetLayerHarness`, `ToolCallDispatcher`, `TableState` dataclass. ~250–400 LOC. Largest new module.
...
| Frontend tool dispatcher | (TS, React-component shaped) | Cannot import from Python | CREATE NEW (small) | New `ToolCallDispatcher` in harness; no existing analog. ~100 LOC |
...
- **AC1.4 wording is unchanged** but DESIGN surfaced that "send a chat turn and assert table state" is more involved than DISCUSS implied (two-step LLM behavior: preview tool → `applyCleaningTransform`). The harness handles this transparently. Recorded as `upstream-changes.md` for traceability — no AC text needs rewriting.
...
2. **DELIVER** (`/nw-deliver`) — Outside-In TDD inside-out from one cleanup op:
   ...
   2. Inner loop 2: `chat_turn` for the simplest op (`Trim whitespace on every text column`) end-to-end including `ToolCallDispatcher`. First chat round-trip green.
