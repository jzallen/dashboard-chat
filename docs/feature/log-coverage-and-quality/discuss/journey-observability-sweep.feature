Feature: Cross-service observability through consistent, correlatable logs
  ...
  # Scenarios follow the operator diagnosis loop in journey-observability-sweep.yaml,
  # in ascending journey-step order: trace (1+2) -> read the failing hop (3) ->
  # read the decision (4) -> raise verbosity (5) -> cross-cutting safety.
  ...
  # --- Steps 1 + 2: get the id and trace it across the stack (US-1, SJ-1) ---
  ...
  # --- Step 3: read the failing hop — the happy path is visible and failures are never silent (US-3, US-5, US-6) ---
  ...
  # --- Step 4: read the decision — auth decisions and access denials, never the credential (US-2, US-4) ---
  ...
  # --- Step 5: raise verbosity at runtime without a redeploy (US-7, SJ-7) ---
  ...
  # --- Cross-cutting safety: redaction guarantee and coexistence with existing log lines (US-7, SJ-7) ---
