...
## DWD-5 — Structural tests authored RED + skip-gated; names left to the implementer

The new structural specs (S1–S6) assert symbols that do not yet exist (pure identity VO, presence `Protocol`, named use-case functions, presenter). Rather than commit speculative RED scaffolds into the flat asset — which the issue's "no new ceremony" + "names at implementer discretion" constraints discourage — each structural test is `@pytest.mark.skip`-gated and imports its target symbol **inside the test body**, so collection stays green and the 62 baseline tests are untouched. DELIVER enables them one at a time and finalises names. S4 (no `statusCode` literal in a use case) and S6 (no `delivery_mode` branch in a use case) are written as **source-introspection** assertions so they remain meaningful regardless of final names.
