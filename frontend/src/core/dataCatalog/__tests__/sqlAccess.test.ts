...
describe("sqlAccess methods", () => {
  ...
  describe("startEnvironment", () => {
    it("sends POST with empty body", async () => {
      const status = { environment_status: "running" };
      mockFetch.mockResolvedValue(mockResponse(status));

      const catalog = createDataCatalog(mockFetch);
      const result = await catalog.startEnvironment("proj-1");

      expect(mockFetch).toHaveBeenCalledWith(
        "/api/projects/proj-1/sql-access/environment/start",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({}),
    ...
    it("throws ApiError on non-ok response", async () => {
      ...
      const catalog = createDataCatalog(mockFetch);
      await expect(catalog.startEnvironment("proj-1")).rejects.toThrow(ApiError);
    });
  });

  describe("stopEnvironment", () => {
    it("sends POST with empty body", async () => {
      const status = { environment_status: "stopped" };
      mockFetch.mockResolvedValue(mockResponse(status));

      const catalog = createDataCatalog(mockFetch);
      const result = await catalog.stopEnvironment("proj-1");

      expect(mockFetch).toHaveBeenCalledWith(
        "/api/projects/proj-1/sql-access/environment/stop",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({}),
    ...
    it("throws ApiError on non-ok response", async () => {
      ...
      const catalog = createDataCatalog(mockFetch);
      await expect(catalog.stopEnvironment("proj-1")).rejects.toThrow(ApiError);
    });
  });
  ...
  describe("restartEnvironment", () => {
    it("sends POST with empty body", async () => {
      ...
      const catalog = createDataCatalog(mockFetch);
      const result = await catalog.restartEnvironment("proj-1");

      expect(mockFetch).toHaveBeenCalledWith(
        "/api/projects/proj-1/sql-access/environment/restart",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({}),
    ...
    it("throws ApiError on non-ok response", async () => {
      ...
      const catalog = createDataCatalog(mockFetch);
      await expect(catalog.restartEnvironment("proj-1")).rejects.toThrow(ApiError);
    });
  });

  describe("getEnvironmentStatus", () => {
    it("fetches environment status", async () => {
      const status = { environment_status: "running", uptime: 3600 };
      mockFetch.mockResolvedValue(mockResponse(status));

      const catalog = createDataCatalog(mockFetch);
      const result = await catalog.getEnvironmentStatus("proj-1");

      expect(mockFetch).toHaveBeenCalledWith(
        "/api/projects/proj-1/sql-access/environment/status",
        {
          method: "GET",
          headers: { "Content-Type": "application/json" },
        },
    ...
    it("throws ApiError on non-ok response", async () => {
      ...
      const catalog = createDataCatalog(mockFetch);
      await expect(catalog.getEnvironmentStatus("proj-1")).rejects.toThrow(ApiError);
    });
  });
});
