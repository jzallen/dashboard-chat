...
The agent service SHALL respond with the AI SDK data stream format via `toDataStreamResponse()`. The custom `{type: "content" | "tool_calls" | "done" | "error"}` SSE event format SHALL be removed.
...
- **AND** the complete tool call (with fully assembled `args`) SHALL be emitted as a single event after streaming is complete
