# Capability: dbt-export-api

HTTP endpoint for authenticated project export, binary response handling via StreamingResponse, and frontend download trigger with blob-based browser download.

This capability covers the API surface, authentication, error handling, and frontend download mechanics. The file generation logic is covered by the `dbt-project-generation` capability.
...
The system SHALL expose a GET endpoint at `/api/projects/{project_id}/export/dbt` that returns a zip archive containing a dbt project generated from the specified project's datasets and transforms.
