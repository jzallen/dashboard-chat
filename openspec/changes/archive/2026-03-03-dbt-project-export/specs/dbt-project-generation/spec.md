# Capability: dbt-project-generation

Backend module that converts a project's metadata, datasets, schemas, and transforms into a complete dbt project file structure (YAML config files, staging model SQL, README) packaged as a zip archive.

This capability covers the file generation logic — what files are produced, their content, and how transforms map to dbt-idiomatic SQL. The API endpoint and download mechanics are covered by the `dbt-export-api` capability.
...
The system SHALL generate a `models/schema.yml` file that declares each dataset as a dbt model with column definitions derived from the dataset's `schema_config`.

- Each model's `name` field SHALL be `stg_{snake_case_dataset_name}` (matching the SQL file name without extension).
- Each model SHALL include a `columns` list derived from the dataset's `schema_config.fields`.
