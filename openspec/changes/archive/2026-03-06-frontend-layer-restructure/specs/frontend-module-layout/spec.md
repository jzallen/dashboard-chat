...
**Reason**: Replaced by the new three-layer architecture (core/ + ui/ + lib/). The flat `lib/` namespace with `shared/`, `auth/`, `chat/`, `dataCatalog/`, `table-tools/`, `raqb/`, and `ui/` subdirectories is dissolved.
**Migration**: Code moves to `core/` (domain logic), `ui/` (React code), or `lib/` (infrastructure) based on the layer-first rules defined in the new "Frontend directory structure" requirement.
