...
**Reason**: The migration from `lib/api/sessions.ts` → `lib/api/chatClient.ts` → `lib/api/chat.ts` is complete. The `sessions.ts` shim and `chatClient.ts` file have been deleted. All consumers now import from `@/chat` directly.
**Migration**: Import from `@/chat` instead of `@/api/sessions` or `@/api/chatClient`.
