...
- Detection SHALL occur on the frontend by checking if `datasetId` is set in the chat context (from channel custom data) before sending to the worker.
...
- Clicking a dataset SHALL:
  1. Set it as the session's dataset context via `channel.updatePartial({ set: { datasetId: "ds-123" } })`.
...
- **GIVEN** the user is in ChatView with no dataset context (channel custom data has no `datasetId`)
...
- **THEN** `channel.updatePartial({ set: { datasetId: "ds-employees" } })` SHALL be called
...
- **GIVEN** the user is in ChatView with dataset "Sales Q4" selected (channel custom data has `datasetId`)
...
- The dataset context SHALL be stored as Stream channel custom data via `channel.updatePartial({ set: { datasetId } })`.
- Navigating to `/table/:datasetId` SHALL update the channel's `datasetId` custom data.
- Returning to ChatView from TableView SHALL retain the dataset context (read from channel custom data on mount).
...
- **THEN** the input gutter SHALL display "Sales Q4" (restored from channel custom data)
