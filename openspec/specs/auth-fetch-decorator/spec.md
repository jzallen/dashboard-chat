# auth-fetch-decorator Specification

## Purpose
Higher-order `withAuth` and `withPreAuth` fetch decorators that provide transport-level auth header injection and 401 retry/refresh logic, decoupling auth concerns from individual API functions.
