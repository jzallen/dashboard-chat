...
# Story 03 (DC-83) — relation_filters normalized (PATTERN-PROVER).
#
...
# cascade (OQ-4) + expand/contract migration reused by stories 04-07. Adding one
# filter is a single-row INSERT; filters are commutative; a parent delete
# cascades only its own rows; loads are org-scoped. Depends on the consolidated
# renderer (story 02). All scenarios @pending until this story lands.
