...
# Story 05 (DC-85) — relation_joins normalized (order honored).
#
...
# order renders the same SQL as the equivalent embedded-array view. Depends on
# the shared component-table repository (story 03). All scenarios @pending until
# this story lands.
...
Feature: Join order is honored through an explicit declaration-ordered sequence
  ...
  Scenario: Joins read in declaration order keep the rendered SQL byte-identical
    ...
    Then the rendered SQL is the same as rendering the equivalent embedded-array view built in the test
