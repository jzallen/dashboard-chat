...
// ═════════════════════════════ the /state actor surface (ADR-046 MR-2) ═════════════════════════════
//
...
//
// It is ADDITIVE — it reuses the SAME ChatAppRuntime as buildChatAppRouter's five
// per-machine mounts (one registry / event log / snapshot store / child resolvers)
// and the same helpers (getActor / coldStart / settle / persist / emptyView /
// onboardingEventSchema / forwardToActor / the failure-sim gate). The whole-actor
// mapper (deriveStateDocument, MR-1) supersedes the per-machine deriveProjection;
// the per-machine surface stays live until MR-7 retires it.
 ...
 *  counter). Reads the same `${child}:${principal}` keys the per-machine appends
 *  write, so /state bookkeeping stays consistent with the legacy wire. */
