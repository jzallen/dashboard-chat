...
stateDiagram-v2
  ...
  loading_session_list --> session_list_visible: loadSessionList onDone [no intent]
  ...
  session_list_visible --> resuming_session: session_clicked (captures session_id into intent_session_id)
  session_list_visible --> session_active_no_messages: new_session_clicked (zeros session_id + transcript + resource + pending_first_message)
  session_list_visible --> loading_session_list: refresh_session_list
  session_list_visible --> loading_session_list: project_ready [project_id changed] (clears all session state)

  resuming_session --> session_list_visible: resumeSession onDone {session_not_found: true} (silent; clears intent_session_id)
  ...
  session_active_no_messages --> session_active_no_messages: new_session_clicked (idempotent no-op)
  session_active_no_messages --> resuming_session: session_clicked
  session_active_no_messages --> creating_session_eagerly: first_message_sent (captures pending_first_message)
  session_active_no_messages --> loading_session_list: project_ready [project_id changed] (clears pending_first_message)

  creating_session_eagerly --> session_active: createSessionEagerly onDone (session_id assigned; transcript + resource zeroed)
  creating_session_eagerly --> error_recoverable: createSessionEagerly onError (transient; last_live_state = session_active_no_messages)
...
| `session_list_visible` | Sidebar populated; no session opened | `loadSessionList` `onDone` (no intent); `resumeSession` `onDone {session_not_found: true}` | `session_clicked`, `new_session_clicked`, `refresh_session_list`, or cross-project `project_ready` |
...
| `session_active_no_messages` | US-206 welcome state; lazily-created session — no backend write fires until first message | `new_session_clicked`; `retry_clicked` (when `last_live_state = session_active_no_messages`) | `first_message_sent`, `session_clicked`, idempotent `new_session_clicked`, or cross-project `project_ready` |
| `creating_session_eagerly` | Invokes `createSessionEagerly` (POST + PATCH title) on the welcome state's first message | `first_message_sent` | `createSessionEagerly` `onDone` / `onError` |
...
| `new_session_clicked` | FE "+ New Session" button | (none) | Enter the welcome state; idempotent from `session_active_no_messages` |
...
| `createSessionEagerly` | `{ project_id, principal_id, first_message }` | `{ session_id }` | On entry into `creating_session_eagerly` |
...
| `project_id` | `string \| null` | `project_ready` | actor inputs; projection | cleared on cross-project `project_ready` |
| `project_name` | `string \| null` | `project_ready` | projection / FE | for project chip render |
...
| `retries` | `number` | `retry_clicked` | observability | bumps each retry |
| `pending_first_message` | `string` | `first_message_sent` | `createSessionEagerly` input on retry | preserved across `session_active_no_messages` ↔ `error_recoverable` (app-arch §6.4 composer-state contract) |
...
- **Receives from orchestrator:** `project_ready` (from project-context's `project_selected` entry — carries `org_id` + `project_id` + `project_name` + forwarded `intent_*`). MR-4's `project_ready` re-broadcast on project switch arrives here too; the cross-project guard (`context.project_id !== event.project_id`) routes it back into `loading_session_list` with all session state cleared per IC-J002-4.
