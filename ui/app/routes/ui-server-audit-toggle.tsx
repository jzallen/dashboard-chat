// /ui-server/projects/:projectId/audit/:auditEntryId — a resource route (action
// only, no component): the ui/ server-side broker for an audit-entry toggle.
...
// forwards the method + JSON body (`{ enabled }`) to the backend
// `/api/projects/{pid}/audit/{auditEntryId}` through auth-proxy via apiFetch
// (which re-verifies the session and injects the identity headers downstream),
// and on success RRv7 auto-revalidates the active loaders so the catalog
// re-derives from server truth.
//
